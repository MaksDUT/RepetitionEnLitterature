...
class SuffixTree:
    ...
    def walk_dfs(self, current, lastcurrent,root):
        start, end = current.start, current.end
        print(end)
        yield self._string[start: end + 1]
        
        #modif
        if lastcurrent ==self.root :
            root = current.start , current.end
        
        if current.leaf :
               # print(lastcurrent.start,lastcurrent.end, lastcurrent.suffixIndex,lastcurrent.suffixLink)
                print(self._string[root[0] : lastcurrent.end +1])
                #print( "---------------",self._string[lastcurrent.start : lastcurrent.end + 1])
        ...
        for node in current.children.values():
            ...
            if node:
                #modif par max 
                #print(node.end,node.suffixIndex)
                
                
                yield from self.walk_dfs(node, current,root)
...
#---------------------------------------------------------------------------------------------------------
#---------------------------------------------------------------------------------------------------------
    def principal_repetition(self, tailleMinPatern) :
        taille = 0
        dicoRepet={}
        dicoInfo={}
        for sub in self.repetition(self.root, self.root, taille, dicoRepet , dicoInfo , tailleMinPatern) :
            continue
    
        return dicoRepet , dicoInfo
        
    def repetition(self , current , lastcurrent , taille, dicoRepet , dicoInfo, tailleMinPatern) :
        sauvegarde = taille
        blackList =[]
        ...
        if current.leaf :
                    ...
                    if self.verifFils(lastcurrent) :
                    
                        taille_fin = current.end - current.start
                        index_end = self.size - taille_fin -1
                        index_start = index_end - taille
                        text = self._string[index_start:index_end]
                        
                        if text not in blackList :
                            
                            
                            if len(text) > tailleMinPatern  :
 
                                if text in dicoRepet.keys() :
                                    ...
                                    if dicoRepet[text] == 2 :
                                        ...
                                        dicoRepet[text] = dicoRepet[text]+1
                                        dicoInfo[text].append((index_start , index_end))
                                else :
                                    dicoRepet[text] = 1
        ...
        if current == self.root :
            ...
            distance = self.edge_length(current) 
            taille = taille + distance
            sauvegarde = taille
        ...
        for node in current.children.values():
            ...
            if node:
                yield from self.repetition(node,current,taille, dicoRepet , dicoInfo, tailleMinPatern) 
                taille = sauvegarde 
    
                
    def verifFils(self,current) :
...
#---------------------------------------------------------------------------------------------------------
    ...
    def print_dfs(self):
        root = self.root
        for sub in self.walk_dfs(self.root, self.root ,root):
            i=1
            #print(sub)
